...
function validateApiKey(req, res, options = {}) {
    ...
    if (!apiKey) {
        // Detect if this is a browser/web request
        ...
        // Browser indicators:
        // 1. User-agent contains browser identifiers
        // 2. Accept header includes text/html (browsers include this)
        // 3. Has origin or referer header from our domain
        ...
        const isWebRequest = origin.includes('localhost') ||
            ...
            origin.includes(process.env.VERCEL_URL || 'never-match') ||
            referer.includes(process.env.VERCEL_URL || 'never-match');

        // Allow if: browser request, web origin, or not explicitly required
        if (isBrowser || isWebRequest || !required) {
            return null;
        }
